 ...
 * 不支持左右滑动
 */
public class CommonTabLayoutBulge extends FrameLayout implements ValueAnimator.AnimatorUpdateListener {
...
private int mSpecialTab; // 标识特殊的tab
/**
 * 用于绘制显示器
 */
...
public CommonTabLayoutBulge(Context context) {
        this(context, null, 0);
        }

public CommonTabLayoutBulge(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
        }

public CommonTabLayoutBulge(Context context, AttributeSet attrs, int defStyleAttr) {
...
// 设置特殊的tab
public void setSpecialTab(int specialTab) {
        mSpecialTab = specialTab;
        }

// 这个方法做适当的调整
public void setCurrentTab(int currentTab) {
        if (mSpecialTab == currentTab) {
        // 这里对特殊的tab处理
        // 目前只是简单的屏蔽了原有的tab事件
        } else {
        mLastTab = this.mCurrentTab;
...
class IndicatorPoint {
    ...
    private CommonTabLayoutBulge.IndicatorPoint mCurrentP = new CommonTabLayoutBulge.IndicatorPoint();
    private CommonTabLayoutBulge.IndicatorPoint mLastP = new CommonTabLayoutBulge.IndicatorPoint();

class PointEvaluator implements TypeEvaluator<CommonTabLayoutBulge.IndicatorPoint> {
    @Override
    public CommonTabLayoutBulge.IndicatorPoint evaluate(float fraction, CommonTabLayoutBulge.IndicatorPoint startValue, CommonTabLayoutBulge.IndicatorPoint endValue) {
        float left = startValue.left + fraction * (endValue.left - startValue.left);
        float right = startValue.right + fraction * (endValue.right - startValue.right);
        CommonTabLayoutBulge.IndicatorPoint point = new CommonTabLayoutBulge.IndicatorPoint();
        point.left = left;
        point.right = right;
        return point;
    }
